...
const ClientOnGoingProjects: React.FC<OnGoingProps> = ({ projectsData, mutate, isLoading, setQuery, error, isEmployee = false }) => {
        ...
        <table>
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr >
                <td className='bg-white' colSpan={5} >{isLoading ? <ReactLoading type={'spin'} color={'#26395e'} height={'20px'} width={'20px'} /> : <p >{t('noDataFound')}</p>}</td>
              </tr>
            )}
          </tbody>
        </table>
