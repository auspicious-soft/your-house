  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        const attachment = { url, type: "Progress", fullName };
      ...
      {attachments?.map((index: any) => {
        ...
        return (
          <div key={index?._id} className="flex justify-between items-center mb-5">
            <div className='flex items-center gap-3' onClick={() => window.open(getImageClientS3URL(index?.url), '_blank')}>
            ...
            <div className="flex gap-[6px] cursor-pointer">
              <div onClick={() => window.open(getImageClientS3URL(index?.url), '_blank')}>
                <DownloadIcon />
              </div>
              <button onClick={() => deleteAttachments(index?._id, index.url)}>
                <DeleteIcon />
              </button>
            </div>
          </div>
